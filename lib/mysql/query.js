  ...
  switch (packet.type) {
    ...
    case Parser.ROW_DATA_PACKET:
      ...
      packet.on('data', function(buffer, remaining) {
        ...
        if (buffer) {
          row[getName(field)] += buffer.toString('utf-8');
        } else {
          row[getName(field)] = null;
        }
        ...
        if (self.typeCast && buffer !== null) {
          switch (field.fieldType) {
            ...
            case Query.FIELD_TYPE_NEWDATE:
              row[getName(field)] = new Date(row[getName(field)]);
              break;
        ...
        if (self._rowIndex == self._fields.length) {
           ...
           for( var name in row ) {
             ...
             if( idx != -1 ) {
                var tName = name.substr(0, idx), cName = name.substr(idx+1);
                if( !row[tName] ) row[tName] = {};
                row[tName][cName] = row[name];
                delete row[name];
             }
           }
           self.emit('row', row);
